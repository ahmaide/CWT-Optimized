...
def discover_images(verbose=True):
    ...
    for base in sorted(base_names):
        IMAGE_LIST.append({
            "base_name": base,
            "filename_32": f"{base}_sigma32.png",
            "paths": {},
            "yolo_paths": {}
        })
        ...
        for s in SCALES:
            ...
            if yolo_y_path.exists():
                IMAGE_LIST[-1]["yolo_paths"][s] = (yolo_y_path, True)
            elif yolo_n_path.exists():
                IMAGE_LIST[-1]["yolo_paths"][s] = (yolo_n_path, False)
            else:
...
def create_page():
    ...
    def show_detection():
        """Show detection banner and overlay."""
        detection_banner.add_class("active")
        page_overlay.add_class("active")

    def hide_detection():
        """Hide detection banner and overlay."""
        detection_banner.remove_class("active")
        page_overlay.remove_class("active")
